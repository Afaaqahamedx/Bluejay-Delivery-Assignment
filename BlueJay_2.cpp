...
// Function to check if an employee worked for more than a specified shift duration
bool workedMoreThanDuration(const Employee& employee, int maxShiftDurationHours) {
    ...
    return hoursWorked > maxShiftDurationHours;
}

// Function to check if an employee has less than a specified time between shifts
bool lessThanTimeBetweenShifts(const Employee& emp1, const Employee& emp2, int minTimeBetweenShifts, int maxTimeBetweenShifts) {
    ...
    return hoursBetween > minTimeBetweenShifts && hoursBetween < maxTimeBetweenShifts;
}
...
int main()
{
    ...
    int maxShiftDurationHours = 14;
    int minTimeBetweenShifts = 1;
    int maxTimeBetweenShifts = 10;

    // Analyze and print results
    for (int i = 0; i < employees.size(); ++i)
    {
        if (workedMoreThanDuration(employees[i], maxShiftDurationHours))
        {
            cout << "Employee " << employees[i].employeeName << " worked for more than " << maxShiftDurationHours << " hours in a single shift." << endl;
        }

        if (i > 0 && lessThanTimeBetweenShifts(employees[i - 1], employees[i], minTimeBetweenShifts, maxTimeBetweenShifts))
        {
            cout << "Employee " << employees[i - 1].employeeName << " and " << employees[i].employeeName << " have less than " << maxTimeBetweenShifts << " hours between shifts but greater than " << minTimeBetweenShifts << " hour." << endl;
        }
